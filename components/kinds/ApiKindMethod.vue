<template>
  <div class="api-item">
    ...
    >
      <span v-if="item.overloads.length > 1" class="muted">{{ index + 1 }}.</span>
      <ApiDocsType :forceText="true" :type="overload" :prefix="prefix ? prefix + methodCallOperator : ''"
      :nested="full"
      />
    </p>
    ...
    <div :is="full ? 'div' : 'details'">
      ...
      <p class="overload-status" v-if="item.overloads.length > 1">📚 Showing overload {{ overloadIndex + 1 }} of {{ item.overloads.length }}</p>
      ...
      <table>
        ...
        <tr v-for="(returnValue, index) in item.returns" :key="index">
...
export default {
  ...
  computed: {
    methodCallOperator () {
      return this.static && API_DOCS.staticMethodCallOperator ||API_DOCS.methodCallOperator || '.'
    },
    static () {
      return !!this.item.static || (this.item.tags && this.item.tags.includes('static'))
    }
  },
  components: {
